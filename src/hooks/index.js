...
export function useExchangeContract(tokenAddress, withSignerIfPossible = true, type) {
  const { library, account } = useWeb3Context()
  ...
  useEffect(() => {
    if (isAddress(tokenAddress)) {
      let stale = false
      getTokenExchangeAddressFromFactory(tokenAddress, library, account, type).then(exchangeAddress => {
        if (!stale) {
...
export function useFactoryContract(tokenAddress) {
  const { library, account } = useWeb3Context()
  ...
  useEffect(() => {
    if (isAddress(tokenAddress)) {
      let stale = false
      getContract(tokenAddress, FACTORY_ABI, library, account).then(exchangeAddress => {
        if (!stale) {
          setExchangeAddress(exchangeAddress)
        }
      })
  ...
  }, [exchangeAddress, library, account])
}
...
export function useAddressBalance(address, tokenAddress) {
  ...
  const updateBalance = useCallback(() => {
    if (isAddress(address) && (tokenAddress === TOKEN_ADDRESSES.ETH || tokenAddress === "ETH" || isAddress(tokenAddress)) && process.env.REACT_APP_ENABLED === "true" ) {
      let stale = false
...
export function useExchangeReserves(tokenAddress) {
  ...
  const reserveTokenMike = useAddressBalance(exchangeContract && exchangeContract.address, tokenAddress)

  return { reserveETH, reserveTokenMike }
}
...
export function useAddressAllowance(address, tokenAddress, spenderAddress) {
  ...
  const updateAllowance = useCallback(() => {
    if (isAddress(address) && isAddress(tokenAddress) && isAddress(spenderAddress)) {
      ...
      getTokenAllowance(address, tokenAddress, TOKEN_ADDRESSES.CC_ROUTER, library)
        .then(allowance => {
          if (!stale) {
