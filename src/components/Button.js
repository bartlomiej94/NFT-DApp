...
import { BurnLoader, NftLoader } from './Loaders'
...
const ButtonFrame = styled.button`
  ...
  height: ${props => typeof props.height === "number" ? props.height + "px" : "48px"};
  width: 100%;
  display: flex;
  align-items: center;
...
export default function Button({ text, onClick = () => {}, preventDefault = true, pending, height, burning, mintingNft, ...rest }) {
  return (
    <ButtonFrame
      onClick={e => {
        if (preventDefault) {
    ...
    >
      {pending ? (
        <CtaTextFlex>
          {text}
          <LoaderWrapper>{burning ? <BurnLoader /> : mintingNft ? <NftLoader /> : null}</LoaderWrapper>
        </CtaTextFlex>
      ) : (
