...
const nameMap = {
  ...
  [timestamp]: 'Time',
  [numberBurned]: 'Tokens Redeemed'
}

// the order for fields that will be submitted
const nameOrder = [name, line1, line2, city, _state, zip, country, email]
  ...
  function updateAutoFields(address) {
    ...
    function getTypes(addressItem, addressVal, item) {
      addressItem.forEach(type => {
        ...
        } else if (Object.keys(item)[0] === _state) {
    ...
    <FormFrame autocomplete="off">
      ...
      <Compressed>
        <input
          ...
          type="text"
          name={_state}
          value={formState[_state]}
          onChange={handleChange}
          placeholder={nameMap[_state]}
      ...
      <ButtonFrame
        type="submit"
        disabled={!canSign || !state.selectedTshirtSize}
        onClick={event => {
          ...
          signer.signMessage(`${header}\n\n${formDataMessage}\n${autoMessage}`).then(returnedSignature => {
            ...
            const finalFormData = {
              ...formState,
              ticker: state.selectedTicker.toUpperCase(),
              size: state.selectedTshirtSize,
              signature: returnedSignature,
              txHash: null,
              burnComplete: false
            }
            
            setState({ ...state, finalFormData })
            setHasConfirmedAddress(true)
          })
      ...
      >
        {canSign ? !state.selectedTshirtSize ? 'Select T-Shirt size' : 'Next' : 'Complete the form to continue'}
      </ButtonFrame>
      <br />
    </FormFrame>
  )
